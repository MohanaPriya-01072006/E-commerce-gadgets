...
import { AppBar, Badge, Box, Button, Container, Drawer, IconButton, InputBase, Stack, Toolbar, Typography, Avatar, Menu, MenuItem, Divider } from '@mui/material';
import { FiMenu, FiSearch, FiShoppingCart, FiUser, FiX, FiLogOut, FiSettings, FiPackage } from 'react-icons/fi';
...
export default function Navbar() {
  ...
  return <>
      ...
      <Container maxWidth={false} sx={{ maxWidth: 1480 }}><Toolbar disableGutters sx={{ minHeight: { xs: 66, md: 82 }, gap: { xs: 1, md: 3 } }}>
        ...
        <Stack direction="row" alignItems="center" divider={<Box sx={{ width: '1px', height: 42, bgcolor: '#dddce5' }} />} spacing={{ xs: .4, md: 2.2 }} sx={{ ml: 'auto' }}>
          ...
          {isAuthenticated ? (
            <>
              {isAdmin && (
                <Button component={RouterLink} to="/admin" sx={{ display: { xs: 'none', lg: 'inline-flex' }, color: '#8e176f', whiteSpace: 'nowrap', fontWeight: 700 }}>
                  Admin Panel
                </Button>
              )}
              <Button onClick={handleMenuOpen} startIcon={
                <Avatar sx={{ width: 32, height: 32, bgcolor: '#8e176f', fontSize: 14, fontWeight: 700 }}>
                  {user?.name?.charAt(0)?.toUpperCase() || 'U'}
                </Avatar>
              } sx={{ color: '#172033', minWidth: 0, px: { xs: .5, md: 1 } }}>
                <Box component="span" sx={{ display: { xs: 'none', md: 'inline' } }}>{user?.name?.split(' ')[0]}</Box>
              </Button>
              <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}
                PaperProps={{ sx: { minWidth: 200, mt: 1, borderRadius: 2, boxShadow: '0 8px 30px rgba(0,0,0,0.12)' } }}>
                <Box sx={{ px: 2, py: 1.5 }}>
                ...
                <MenuItem onClick={() => { handleMenuClose(); navigate('/my-orders'); }}>
                  <FiPackage style={{ marginRight: 10 }} /> My Orders
                </MenuItem>
                {isAdmin && (
                  <MenuItem onClick={() => { handleMenuClose(); navigate('/admin'); }}>
                    <FiSettings style={{ marginRight: 10 }} /> Admin Panel
                  </MenuItem>
                )}
                <Divider />
    ...
    <Drawer anchor="right" open={drawerOpen} onClose={() => setDrawerOpen(false)} PaperProps={{ sx: { width: 300, p: 2.5 } }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}><Typography sx={{ color: '#3e0b55', fontWeight: 800, fontSize: 26 }}>moprix</Typography><IconButton onClick={() => setDrawerOpen(false)}><FiX /></IconButton></Stack>
      <Box component="form" onSubmit={submitSearch} sx={{ display: 'flex', alignItems: 'center', border: '1px solid #c9c7d0', borderRadius: 1, p: 1, mb: 2.5 }}><FiSearch /><InputBase value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search gadgets" sx={{ ml: 1, flex: 1 }} /></Box>
      <Stack spacing={.8}>
        {categories.map((item) => <Button component={RouterLink} to="/shop" onClick={() => setDrawerOpen(false)} key={item} sx={{ justifyContent: 'flex-start', color: '#172033' }}>{item}</Button>)}
        <Box sx={{ borderTop: '1px solid #e6e5ea', my: 1 }} />
        {isAuthenticated ? (
          <>
            <Box sx={{ px: 1, py: 1 }}>
              <Typography sx={{ fontWeight: 700 }}>{user?.name}</Typography>
            ...
            <Button component={RouterLink} to="/my-orders" onClick={() => setDrawerOpen(false)} variant="outlined">My Orders</Button>
            {isAdmin && <Button component={RouterLink} to="/admin" onClick={() => setDrawerOpen(false)} variant="outlined" sx={{ color: '#8e176f', borderColor: '#8e176f' }}>Admin Panel</Button>}
            <Button onClick={() => { logout(); setDrawerOpen(false); navigate('/'); }} variant="outlined" color="error">Logout</Button>
          </>
        ) : (
          <>
            <Button component={RouterLink} to="/login" onClick={() => setDrawerOpen(false)} variant="outlined">Login</Button>
            <Button component={RouterLink} to="/signup" onClick={() => setDrawerOpen(false)} variant="outlined">Sign Up</Button>
          </>
        )}
        <Button component={RouterLink} to="/enquiry" onClick={() => setDrawerOpen(false)} variant="contained" sx={{ bgcolor: '#8e176f' }}>Enquiry</Button>
      </Stack>
    </Drawer>
  </>;
}
